...
class MyWebServer(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        #copied from the mime library: https://docs.python.org/2/library/mimetypes.html
        mimetypes.init()
        ...
        #written by desperatelearner(http://stackoverflow.com/users/1253102/desperatelearner) on 
        ...
        if listofdata[0]=="GET":
            st = "/www"+listofdata[1]              
            if "." not in listofdata[1]:
                if listofdata[1].endswith("/"):
                    st = st+"index.html"
                else:
                    st = st+"/index.html"
            try:
                ...
                #written by danben(http://stackoverflow.com/users/217332/danben)
                #on http://stackoverflow.com/questions/2104080/how-to-check-file-size-in-python
                self.request.sendall("Content-Length: "+str(os.path.getsize(path))+";")


                
                self.request.sendall("Transfer-Encoding: chunked;\r\n\r\n")

                #file read loop written by http://stackoverflow.com/users/2551338/leandro-gs
                ...
                self.request.sendall("NOT FOUND")
        else:
